...
class CompanyRepository {
    public function store($data) {
        try {
            ...
            if ($data['id']) {
                ...
                $company = new Company();
                $company->app_id = Auth::user()->app_id;
            }
            ...
            if (!$data['id']) {
                $this->createDefaultCompany($company);
            }
    ...
    public function createDefaultCompany($company) {
        ...
        $coaPurchase->is_locked = 1;
        $coaPurchase->save();

        // Default for comission
        $coaPurchase = new Coa();
        $coaPurchase->company_id = $company->id;
        $coaPurchase->category_id = $company->type === 'pt' ? 36 : 81;
        $coaPurchase->posting_id = 6;
        $coaPurchase->account_code = "511101-Comission";
        $coaPurchase->account_number = 511101;
        $coaPurchase->account_name = "Comission";
