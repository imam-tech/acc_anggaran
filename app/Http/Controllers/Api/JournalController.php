...
class JournalController extends Controller {
    ...
    public function balanceProfit(Request $request) {
        return response()->json($this->journalRepo->balanceProfit($request->all()));
    }

    public function cashflow(Request $request) {
        return response()->json($this->journalRepo->cashflow($request->all()));
    }

    public function cashflowInitial(Request $request) {
        return response()->json($this->journalRepo->cashflowInitial($request->all()));
    }

    public function journal(Request $request) {
        return response()->json($this->journalRepo->journal($request->all()));
    }

    public function generalLedger(Request $request) {
        return response()->json($this->journalRepo->generalLedger($request->all()));
    }
