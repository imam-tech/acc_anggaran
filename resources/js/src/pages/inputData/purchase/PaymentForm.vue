    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-title">
                ...
                <router-link :to="'/app/input-data/purchase/'+$route.params.id+'/detail'" class="btn btn-success float-right mt-3 mr-3">
        ...
        methods: {
            ...
            async getDataOther() {
                try {
                    ...
                    this.payFroms = await this.$axios.get(`api/master-data/coa?is_active=1&coa_name=KAS`)
            ...
            async handleSubmitAddNewPaymentMethod() {
                try {
                    this.$vs.loading()
                    const resp = await this.$axios.post(`api/master-data/setting/payment-method`, this.formAddPaymentMethod)
                    this.$vs.loading.close()
                    if (!resp.status)  {
                        Swal.fire({
                            position: 'top',
                        ...
                        this.paymentMethods = await this.$axios.get(`api/master-data/setting/payment-method`)
                    }
                } catch (e) {
                    this.$vs.loading.close()
                    Swal.fire({
            ...
            async handleGet() {
                try {
                    this.$vs.loading()
                    const purchaseLocal = await this.$axios.get(`api/input-data/purchase/${this.$route.params.id}/detail`)
                    this.$vs.loading.close()
            ...
            async handleGetPayment() {
                try {
                    ...
                    const paymentLocal = await this.$axios.get(`api/input-data/purchase/payment/${this.$route.params.type}/detail`)
                    this.$vs.loading.close()
                    if (paymentLocal.status) {
                        if (paymentLocal.data.status) {
                            Swal.fire({
                                ...
                                this.$router.push('/app/input-data/purchase/'+this.$route.params.id+'/detail')
                            })
                            return
                        }
            ...
            async handleSubmit() {
                ...
                try {
                    this.$vs.loading()
                    const resp = await this.$axios.post(`api/input-data/purchase/payment`,
                        formData, {
                            headers: {
                    ...
                    if (!resp.status)  {
                        ...
                        this.$router.push(`/app/input-data/purchase/payment/${resp.data.id}/detail`)
                    }
                } catch (e) {
                    this.$vs.loading.close()
                    Swal.fire({
