    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-title">
                ...
                <router-link v-if="formData.id === ''" to="/app/input-data/sales" class="btn btn-success float-right mt-3 mr-3">
                    <i class="fas fa-arrow-left"></i> Back
                </router-link>
                <router-link v-else :to="'/app/input-data/sales/'+formData.id+'/detail'" class="btn btn-success float-right mt-3 mr-3">
                    <i class="fas fa-arrow-left"></i> Back
                </router-link>
            </div>
            <div class="card-body">
                            ...
                            <div class="modal-body">
                                ...
                                ">*) required</span></label>
                                    ...
                                    <div class="col-lg-12">
                                        ...
                                        <v-select v-model="formAddCustomer.sale_account_id" :options="coas" :reduce="p => p.id" style="width: 100%" label="account_code">
    ...
    export default {
        ...
        methods: {
            ...
            async handleGet() {
                try {
                    this.$vs.loading()
                    const salesLocal = await this.$axios.get(`api/input-data/sales/${this.$route.params.type}/detail`)
                    ...
                    if (salesLocal.data.paid_date) {
                        Swal.fire({
                            ...
                            this.$router.push(`/app/input-data/sales/${salesLocal.data.id}/detail`);
                        })
                        return
                    }
                    ...
                    salesLocal.data.tag_details.forEach((x) => {
            ...
            async handleSubmit() {
                ...
                try {
                    this.$vs.loading()
                    const resp = await this.$axios.post(`api/input-data/sales`,
                        formData, {
                            headers: {
                    ...
                    if (!resp.status)  {
                        ...
                        this.$router.push(`/app/input-data/sales/${resp.data.id}/detail`)
                    }
                } catch (e) {
                    this.$vs.loading.close()
                    Swal.fire({
            ...
            async handleSubmitAddNewCustomer() {
                try {
                    this.$vs.loading()
                    const resp = await this.$axios.post(`api/master-data/contact`, this.formAddCustomer)
                    this.$vs.loading.close()
                    if (!resp.status)  {
                        Swal.fire({
                            position: 'top',
            ...
            async getDataOther() {
                try {
                    this.$vs.loading()
                    this.customers = await this.$axios.get(`api/master-data/contact?type=customer&is_archive=no`)
                    const prodLocals = await this.$axios.get(`api/master-data/product?is_sale=1&is_archive=no`)
