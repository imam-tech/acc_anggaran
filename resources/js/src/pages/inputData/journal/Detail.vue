    ...
    export default {
        ...
        methods: {
            ...
            async handleApproveReject(type) {
                ...
                Swal.fire({
                    ...
                    if (isConfirm.isConfirmed) {
                        try {
                            this.$vs.loading()
                            const respSave = await this.$axios.post(`api/input-data/journal/${this.$route.params.id}/approval`, {
                                status: type,
            ...
            async getData() {
                try {
                    this.$vs.loading()
                    const respDe = await this.$axios.get(`api/input-data/journal/${this.$route.params.id}/detail`)
                    this.$vs.loading.close()
                    if (!respDe.status) {
                        Swal.fire({
                            position: 'top-end',
