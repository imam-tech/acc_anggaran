            ...
            <div class="card-title">
                <div class="row m-3">
                    ...
                    <div class="col-6 text-right">
                        <router-link to="/app/input-data/journal/form/create">
                ...
                <div class="table-responsive">
                    <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tr v-for="(journal, index) in journals" :key="index">
                            <td>
                                <a :href="'/app/input-data/journal/' + journal.id + '/detail'" target="_blank">
                            ...
                            <td class="text-right">
                                <router-link :to="'/app/input-data/journal/' + journal.id + '/detail'" target="_blank">
                                    <button type="button" class="btn btn-primary">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                </router-link>
                                <router-link v-if="journal.approved_at === null && journal.rejected_at === null" :to="'/app/input-data/journal/form/' + journal.id">
    ...
    export default {
        ...
        methods: {
            ...
            async getData() {
                try {
                    this.$vs.loading()
                    this.journals = await this.$axios.get(`api/input-data/journal?start_date=${this.filter.start_date}&end_date=${this.filter.end_date}
            ...
            async handleDelete(id) {
                Swal.fire({
                    ...
                    if(result.isConfirmed == true){
                        this.$vs.loading()
                        this.$axios.delete(`api/input-data/journal/${id}/delete`).then((response)=>{
                            this.$vs.loading.close()
                            if(response.status){
                                Swal.fire({
                                    icon: 'success',
