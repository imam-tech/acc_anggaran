            ...
            <div class="card-title">
                <div class="mt-3 d-flex justify-content-between">
                    ...
                    <button @click="handleShowCrudModal()" type="button" class="btn btn-success float-right mr-3">
                        <i class="fa fa-plus-circle"></i> Create New Product Category
                    </button>
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tr v-else v-for="(p, pI) in productCategories" :key="pI">
                            <td>
                    ...
                    <form @submit.prevent="handleSubmit">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                <label>Category Title<span style="
                                    color: red;
                                    font-weight: bold;
                                    font-style: italic;
                                ">*) required</span></label>
    ...
    export default {
        ...
        methods: {
            ...
            async getData() {
                try {
                    this.$vs.loading()
                    this.productCategories = await this.$axios.get(`api/product/category`)
                    this.$vs.loading.close()
                } catch (e) {
                    this.$vs.loading.close()
                    Swal.fire({
            ...
            async handleSubmit() {
                try {
                    this.$vs.loading()
                    const resp = await this.$axios.post(`api/product/category`, this.formData)
                    this.$vs.loading.close()
                    if (!resp.status)  {
                        Swal.fire({
                            position: 'top',
                        ...
                        this.$axios.delete(`api/product/category/${id}/delete`).then((response)=>{
                            ...
                            if(response.status){
                                ...
                                Swal.fire({
                                    icon: "error",
                                    title: "Opps...",
                                    text: "Failed To Delete Product Category : " + response.message ,
                                    allowOutsideClick: false,
                                    allowEscapeKey: false,
                                    allowEnterKey: false,
                                });
