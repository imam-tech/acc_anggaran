    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-title">
                <h1 class="h3 mt-3 ml-3 text-gray-800 float-left">Supplier Form</h1>
                <router-link to="/app/purchase/suppliers" class="btn btn-success float-right mt-3 mr-3">
    ...
    export default {
        ...
        methods: {
            async handleSubmit() {
                ...
                try {
                    this.$vs.loading()
                    const resp = await this.$axios.post(`api/purchase/supplier`, this.formData)
                    ...
                    if (!resp.status)  {
                        ...
                        this.$router.push('/app/purchase/suppliers')
                    }
                } catch (e) {
                    this.$vs.loading.close()
                    Swal.fire({
            ...
            async getData() {
                try {
                    this.$vs.loading()
                    const resp = await this.$axios.get(`api/purchase/supplier/${this.$route.params.type}/detail`)
                    this.$vs.loading.close()
                    if (!resp.status) {
                        Swal.fire({
                            position: 'top',
