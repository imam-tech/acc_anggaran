    ...
    export default {
        ...
        methods: {
            ...
            async handleSubmit(month) {
                try {
                    this.$vs.loading()
                    const respData = await this.$axios.get('api/output-data/report/journal?status=' + this.filter.status + '&start_date=' + this.filter.start_date + '&end_date=' + this.filter.end_date)
