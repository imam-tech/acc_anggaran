    ...
    export default {
        ...
        methods: {
            ...
            async getData() {
                try {
                    this.$vs.loading()
                    this.users = await this.$axios.get(`api/configuration/user?name=${this.formFilter.name}&email=${this.formFilter.email}&role=${this.formFilter.role}`)
            ...
            async submitChangePasswordUser() {
                try {
                    this.$vs.loading()
                    const respSave = await this.$axios.post('api/configuration/user/change-password', this.formChangePassword)
                    this.$vs.loading.close()
                    if (!respSave.status) {
                        Swal.fire({
                            position: 'top-end',
            ...
            async submitUser() {
                try {
                    ...
                    this.$vs.loading()
                    const respSave = await this.$axios.post('api/configuration/user', this.formData)
                    this.$vs.loading.close()
                    if (!respSave.status) {
                        Swal.fire({
                            position: 'top-end',
            ...
            async handleDelete(id) {
                Swal.fire({
                    ...
                    if(result.isConfirmed == true){
                        this.$vs.loading()
                        this.$axios.delete(`api/configuration/user/${id}/delete`).then((response)=>{
                            this.$vs.loading.close()
                            if(response.status){
                                Swal.fire({
                                    icon: 'success',
