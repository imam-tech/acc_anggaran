    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-title">
                ...
                <router-link :to="'/app/configuration/company/'+$route.params.id+'/detail'" class="btn btn-success float-right mt-3 mr-3">
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tr v-for="(transaction, key) in transactions" :key="key">
                            ...
                            <td class="text-right">
                                <button type="button" @click="handleReject(transaction.id)" class="btn btn-danger">
        ...
        mounted() {
            if (!this.$store.state.permissions.includes('transaction_push_plugin')) {
                Swal.fire({
                    ...
                    this.$router.push('/app/configuration/company/' + this.$route.params.id + '/detail')
                })
                return
            }
            this.getData()
        ...
        methods: {
            handlePushPlugin() {
                ...
                Swal.fire({
                    ...
                    if(result.isConfirmed == true){
                        this.$vs.loading()
                        this.$axios.post(`api/configuration/company/${this.$route.params.id}/push-plugin`, params).then((response)=>{
                            ...
                            if(response.status){
                                ...
                                Swal.fire({
                                    icon: "error",
                                    title: "Opps...",
                                    text: "Failed To Push : " + response.message ,
                                    allowOutsideClick: false,
                                    allowEscapeKey: false,
                                    allowEnterKey: false,
                                });
            ...
            handleReject(itemId) {
                ...
                Swal.fire({
                    ...
                    if(result.isConfirmed == true){
                        this.$vs.loading()
                        this.$axios.get(`api/transaction/${itemId}/reject`).then((response)=>{
                            this.$vs.loading.close()
                            if(response.status){
                                Swal.fire({
                                    icon: 'success',
                                ...
                                Swal.fire({
                                    icon: "error",
                                    title: "Opps...",
                                    text: "Failed To Reject Coa : " + response.message ,
                                    allowOutsideClick: false,
                                    allowEscapeKey: false,
                                    allowEnterKey: false,
                                });
